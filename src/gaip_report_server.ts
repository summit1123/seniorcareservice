...
const CARE_NARRATIVE_SCHEMA = {
  ...
  properties: {
    ...
    analyst_report_ko: {
      type: "string",
      description: "종합 소견 — 담당 심사역이 동료에게 이 고객을 설명하듯 쓰는 4~6문단(문단 사이 빈 줄 \\n\\n). 각 문단은 **볼드 리드인**으로 시작: **이번 달 운전.** 이 분이 어떻게 다녔는지 → **우려 지점.** 무엇이 왜 걱정되는지 → **우대 축 판단.** / **케어 축 판단.** 수치 근거 → **권고와 다음 달.** 이 사람에게 왜 이 지원이 맞는지와 관찰 포인트. '안내 예정입니다' 같은 행정 상투어 금지, 자연스러운 심사역 문장으로. 마크다운은 볼드(**)만, 입력 수치는 그대로 인용."
    },
    xai_notes: {
      type: "array",
      items: {
...
export async function generateCareNarrative(localReport: JsonRecord, repoRoot?: string): Promise<JsonRecord> {
  ...
  const systemPrompt = [
    ...
    "케어 판정 서술 규칙: 감지는 직전 2개월 대비 급변, 유지는 원래 생활(학습된 생활권) 복귀 전까지입니다. '첫 2개월 기준선에 고정해 판정한다'는 서술은 금지 — 첫 2개월은 생활권 학습 기간으로만 언급하세요.",
