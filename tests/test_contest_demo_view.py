...
class TestContestDemoView(unittest.TestCase):
    def test_contest_demo_page_shows_data_policy_and_llm_status(self) -> None:
        ...
        self.assertIn("이미 만든 데이터로 해보는 판정 계산기", html)
        self.assertIn("고객과 최근 조건을 바꾼 가정 계산", html)
        self.assertIn("새 90일 주행 로그를 다시 만들거나 Agent/OpenAI를 재호출하는 영역이 아닙니다", html)
        self.assertIn("가정값으로 판정 계산", html)
