...
class TestContestDemoView(unittest.TestCase):
    def test_contest_demo_page_shows_data_policy_and_llm_status(self) -> None:
        ...
        self.assertIn("먼저 30명 전체를 봅니다", html)
        self.assertIn('data-customer-population-count="30"', html)
        self.assertIn("cust_001", html)
        self.assertIn("cust_030", html)
        self.assertIn("선택 고객으로 보는 판정 흐름", html)
        self.assertIn("적게 탔습니다", html)
        self.assertIn("그래서 기존은 할인", html)
        self.assertIn("하지만 최근 변화가 있습니다", html)
        self.assertIn("제안 방식 판정", html)
        ...
        self.assertIn("고객별 판정 확인", html)
        self.assertIn("고객 선택", html)
        self.assertIn("숫자 입력은 기본 시연에서는 숨기고", html)
        self.assertIn("선택 고객의 숫자를 임시로 바꿔보기", html)
        self.assertIn("선택 고객 보기", html)
        self.assertIn("임시값으로 계산", html)
        self.assertNotIn("새 90일 주행 로그를 다시 만들거나 Agent/OpenAI를 재호출하는 영역이 아닙니다", html)
    ...
    def test_contest_demo_simulation_query_changes_selected_customer_and_conditions(self) -> None:
        ...
        self.assertIn("합성고객 001의 90일 주행 좌표", html)
