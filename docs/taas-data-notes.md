...
TAAS는 AI가 개인별 사고를 예측했다는 근거가 아닙니다. 이 제안에서 TAAS의 역할은 “시니어 안심주행 특약이 왜 필요한가”와 “어떤 위험행동을 score에서 더 무겁게 볼 것인가”를 뒷받침하는 공공 통계 근거입니다.

| 사용 위치 | 사용 방식 | 사용하지 않는 방식 |
|---|---|---|
| 상품 필요성 | 고령운전사고, 야간사고, 법규위반 항목을 배경 근거로 요약 | 특정 고객의 사고 가능성 라벨로 사용하지 않음 |
| score weight | 과속, 안전거리 미확보, 안전운전 의무 불이행, 야간 치사율 보정을 참고 | 지자체별 사고 건수를 고객 위치 위험도로 직접 매칭하지 않음 |
| 제안서 문장 | “공공 사고통계 기반 위험행동 가중치”로 표현 | “TAAS로 사고를 예측한다”는 표현 금지 |
